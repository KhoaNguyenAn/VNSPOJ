...
const int N = 1e4 + 6;
long long n,ok,res=0,a[100][N],check[1000][1000],tmp,f[100][N],sl,b[N];
...
{
	long long ans = 0;
	for (int ii = 1; ii <= 8; ii++) ans+= getbit(t,ii)*a[ii][c];
	return ans;
}
int main()
{
	...
	for (int i = 1; i <= sl;i++)
	 for (int k = 1; k <= sl; k++)
			check[i][k] = kt(b[i],b[k]);
	for (int i = 1; i <= sl; i++) f[i][1] = sum(1,b[i]);
	for (int j = 2; j <= n; j++)
	{
		for (int i = 1; i <= sl; i++)
		{
	...
	for (int i = 1; i <= sl; i++) res = max(res,f[i][n]);
	cout << res;
	#ifdef HOME
